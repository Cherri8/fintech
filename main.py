    ...
    try:
        ...
        if results:
            for result in results:
                parts = result.split('#')
                if len(parts) < 4:  # Skip invalid entries
                    continue
                
                if parts[2] == 'Top Up':
                    if parts[4] == username:  # User is recipient
                        wallet += int(parts[3])  # Amount
                elif parts[2] == 'Transfer':
                    try:
                        amount = int(parts[4])  # Amount field
                        sender = parts[5]
                        receiver = parts[6] if len(parts) > 6 else None
                        
                        if sender == username:  # User is sender
                            wallet -= amount
                        elif receiver == username:  # User is recipient
                            wallet += amount
                    except (ValueError, IndexError):
                        continue  # Skip invalid transfer entries
            
        return wallet
    except Exception as e:
        print(f"Error calculating wallet: {e}")
        return 0
...
database_dir = r'C:\Users\chara\Downloads\Fintech-Crypto project\database'

# Absolute paths for user.txt and history.txt
    ...
    try:
        ...
        if data:  # Only decrypt if there's data
            try:
                ...
                with open(HISTORY_PATH, 'w') as history_obj:
                    history_obj.write(plaintext)
            except Exception as e:
                print(f"Error decrypting history.txt: {e}")
    except Exception as e:
        print(f"Database decryption error: {e}")

# Retrieve the secret key and run the decryption function
SECRET_KEY = get_key()
decryptDatabase()
                    ...
                    if is_login:
                        ...
                        while True:
                            ...
                            if main_chc == '1':
                                ...
                                string = f"{_datetime}#Top Up#{amount}#{username}"
                                ...
                                if chc == '1':
                                    ...
                                    if not is_back_to_menu :
                                        ...
                                        if match(USER_PATH,username,pin,'PIN'):
                                            ...
                                            _datetime = str(datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
                                            string = f"{_datetime}#Transfer#TRANSFER#{amount}#{username}#{receiver[1]}"
